...
def initial_setup(
    alice, bob, chain, cortex_gauge, reward_contract, coin_reward, coin_a, coin_b, mock_lp_token
):
...
def test_claim_one_lp(bob, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(WEEK)
...
def test_claim_updates_claimed_reward(bob, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(WEEK)
    ...
    for coin in (coin_a, coin_b):
        ...
        assert reward <= REWARD
        assert approx(REWARD, reward, 1.001 / WEEK)  # ganache-cli jitter of 1 s
        assert cortex_gauge.claimed_reward(bob, coin) == reward


def test_claim_for_other(bob, charlie, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(WEEK)

    cortex_gauge.withdraw(LP_AMOUNT, {"from": bob})
    cortex_gauge.claim_rewards(bob, {"from": charlie})
...
def test_claim_for_other_no_reward(bob, charlie, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(WEEK)
    cortex_gauge.claim_rewards(charlie, {"from": bob})
...
def test_claim_two_lp(
    alice, bob, chain, cortex_gauge, mock_lp_token, coin_a, coin_b, reward_contract, no_call_coverage
):
    ...
    for coin in (coin_a, coin_b):
        ...
        assert coin.balanceOf(cortex_gauge) == 0


def test_claim_duration(bob, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(86400)
    cortex_gauge.claim_rewards({"from": bob})

    claim_time = cortex_gauge.last_claim()
    ...
    assert math.isclose(cortex_gauge.last_claim(), claim_time, abs_tol=1)
    assert claimed == [i.balanceOf(bob) for i in (coin_a, coin_b)]

    chain.sleep(1801)
    cortex_gauge.claim_rewards({"from": bob})

    assert math.isclose(cortex_gauge.last_claim(), chain[-1].timestamp, abs_tol=1)
    assert chain[-1].timestamp > claim_time
    assert coin_a.balanceOf(bob) > claimed[0]
    assert coin_b.balanceOf(bob) > claimed[1]


def test_claim_set_alt_receiver(bob, charlie, chain, cortex_gauge, coin_a, coin_b):
    chain.sleep(WEEK)

    cortex_gauge.claim_rewards(bob, charlie, {"from": bob})
...
def test_claim_for_other_changing_receiver_reverts(bob, charlie, chain, cortex_gauge):
    chain.sleep(WEEK)

    cortex_gauge.withdraw(LP_AMOUNT, {"from": bob})
    with brownie.reverts("dev: cannot redirect when claiming for another user"):
        cortex_gauge.claim_rewards(bob, charlie, {"from": charlie})
