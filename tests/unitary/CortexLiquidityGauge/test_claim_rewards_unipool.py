...
def test_claim_one_lp(bob, chain, cortex_gauge, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_for_other(bob, charlie, chain, cortex_gauge, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_for_other_no_reward(bob, charlie, chain, cortex_gauge, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_two_lp(alice, bob, chain, cortex_gauge, mock_lp_token, coin_reward, no_call_coverage):
    ...
    # Calculate rewards
    claimable_rewards = [
        cortex_gauge.claimable_reward_write.call(acc, coin_reward, {"from": acc})
...
def test_claimable_rewards_write_retrieves_rewards(
    alice, chain, cortex_gauge, mock_lp_token, coin_reward, no_call_coverage
):
    ...
    before = coin_reward.balanceOf(cortex_gauge)
    cortex_gauge.claimable_reward_write(alice, coin_reward, {"from": alice})
    assert before < coin_reward.balanceOf(cortex_gauge)
