...
def initial_setup(
    ...
    for acct in accounts[:10]:
        cortex_gauge.deposit(10 ** 18, acct, {"from": alice})
...
def test_mass_withdraw_claim_rewards(accounts, cortex_gauge, coin_reward, mock_lp_token):
    for account in accounts[:10]:
        cortex_gauge.withdraw(cortex_gauge.balanceOf(account), {"from": account})
        assert cortex_gauge.claimed_reward(account, coin_reward) == 0
        assert cortex_gauge.claimable_reward_write.call(account, coin_reward) > 0

    for account in accounts[:10]:
        cortex_gauge.claim_rewards({"from": account})
